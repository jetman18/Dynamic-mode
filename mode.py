...
air_density = 1.293 
wing_area = 0.21 
wing_ctrl_area = 0.015
Cd_o = 0.14
max_aileron_angle = 20
aileron_Cl = 0.011
aileron_Cd = 0.0013
...
ctrl_right = 0
step = 0
#def loop(dt,roll,pitch):

f = open("data.txt", "w")
last_t = 0
def loop(dt,ch2,ch3):
    global yaw,vx,vy,vz,alpha,T,step,roll,pitch,yaw,last_t
    ...
    #Cm_p = 0.0001*alpha + Cm_o
    Cm_p = (0.002*pow(alpha,3) + 0.2*alpha)*0.0002
    #Cm_p = 0.001*atan(alpha*toRad) + Cm_o
    '''
    if Cm_p > 0.8:
        Cm_p = 0.8
    elif Cm_p < -0.8:
